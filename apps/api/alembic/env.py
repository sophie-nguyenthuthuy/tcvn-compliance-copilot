...
Async-aware: uses the same asyncpg URL as the app, runs migrations inside an
`async_engine.begin()` block. The metadata is the unified `Base.metadata` so
that all model imports happen via `tcvn_copilot.db.models`.
"""
